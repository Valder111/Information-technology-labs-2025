...
Первым шагом была проверка установки Docker на виртуальной машине Debian. 

> `docker --version`

Команда подтвердила наличие установленного Docker версии 25.1.5, что означало готовность системы к дальнейшей работе.
...
После подготовки репозитория был скачан базовый образ nginx версии 1.21.1.

> `docker pull nginx:1.21.1`

Этот образ послужил основой для создания кастомной версии.
...
Следующим этапом стала непосредственная сборка кастомного образа.

> `docker build -t teoxxid/custom-nginx:1.0.0 .`
...
Перед публикацией образа была выполнена аутентификация в Docker Hub.
...
> `docker push teoxxid/custom-nginx:1.0.0`
...
Первым этапом был запуск контейнера с использованием ряда ключевых параметров.

> `docker run -d --name teoxxid-custom-nginx-t2 -p 127.0.0.1:8080:80 teoxxid/custom-nginx:1.0.0`
...
> `docker rename teoxxid-custom-nginx-t2 custom-nginx-t2`

Исходное имя "teoxxid-custom-nginx-t2" было изменено на унифицированное "custom-nginx-t2" в соответствии с требованиями задания. Данная операция была выполнена без остановки или удаления контейнера, что демонстрирует возможность динамического управления работающими контейнерами.
...
Проверка статуса через:

> `docker ps -a`

Подтвердила изменение состояния контейнера на "Exited". Анализ показал, что контейнер остановился по причине получения основным процессом nginx сигнала SIGINT от комбинации Ctrl-C, что является стандартным поведением для демонических процессов в Linux.
...
<p align="center">
  <img src="media/image038.png" alt="Переименование образа">
</p>

<p align="center">
  <img src="media/image039.png" alt="Загрузка образа в registry">
</p>
